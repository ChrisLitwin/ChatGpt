...
def baseSize =
    ...
    else if isTue then 0.80
    else if isMon then 0.50
    else if isWed then 0.30
    else 0.50;
...
# Pre-open label using the palette
def hardPass =
    (isWed and !BigSurprise) or
...
AddLabel(ShowPreOpenPanel and preOpen,
    ...
    (if hardPass then GlobalColor("SigBad")                          # PASS -> red
     else if softCaution or sizeSuggest < 0.9 then GlobalColor("SigNotGood")  # SIZE DOWN -> pink
     else GlobalColor("SigBetter")));                                # GO -> green
...
def regimeCodeTue =
    if atrUsed < 0.70 then (if hit10 then 3 else 0)
    else if atrUsed > 0.90 then 4
    else if hit20 then 2 else 0;

def regimeCodeWed =
    if atrUsed >= ATR_LowBand and atrUsed <= ATR_MidBand then 1
    else if atrUsed > ATR_HiBandLow and atrUsed <= ATR_HiBandHigh then 1
    else if hit20 then 2 else 0;
...
def regimeCodeFri =
    ...
    else if atrUsed > ATR_HiBandLow then 4
    else if hit20 then 2 else 0;
...
def friExit = isFri and afterNoon and atrUsed > ATR_HiBandLow;
def tueCont = isTue and atrUsed < 0.70 and hit10;
def wedFade = isWed and atrUsed >= ATR_LowBand and atrUsed <= ATR_MidBand;
def monQuiet = isMon and atrUsed < ATR_LowBand;
def thuContStrong = isThu and (thuStick or hit20);

def promptCode =
    if thuContStrong then 10
    else if friExit then 1
...
AddLabel(ShowPromptLabel and !preOpen,
    "Prompt: " +
    (if promptCode == 10 then "Thu: Open/Hold continuation spread (sticks ≥ " + AsPrice(ThuStickPts) + ")"
     else if promptCode == 9 then "Tue: Continuation (ATR<70% & ±10 hit)"
     else if promptCode == 8 then "Continuation: Might wanna take profits"
     else if promptCode == 7 then "Strong continuation: Stay Calm/Target Profit"
     else if promptCode == 3 then "Fade extremes / Early Reversion likely"
     else if promptCode == 2 then "Standby / Stay Alert"
     else if promptCode == 1 then "Stall/Exit: Avoid new entries, manage/close"
     else "Wait for ±10 or clearer ATR"),
    (if promptCode >= 8 then GlobalColor("PromptLean")     # better
     else if promptCode == 7 then GlobalColor("PromptGo")  # good
     else if promptCode == 3 then GlobalColor("PromptFade")
...
def signalCode =
    if isThu and (thuStick or hit20) then 2
    else if isFri and afterNoon and atrUsed > ATR_HiBandLow then -2
...
AddLabel(ShowSignalLabel and !preOpen,
    ...
    (if signalCode == 2 then "CONTINUATION"
     ...
     else if signalCode == -1 then "Early Risky Mover"
     else if signalCode == -2 then "STALL / EXIT"
     else "Standby"),
    (if signalCode == 2 then GlobalColor("SigBetter")     # green (better)
     else if signalCode == 1 then GlobalColor("SigGood")  # dark green (good)
     else if signalCode == -2 then GlobalColor("SigBad")  # red (bad)
     else if signalCode == -1 then GlobalColor("SigNotGood")  # pink (fade)
     else Color.WHITE));                                   # standby = white
...
# ±10 are identical “good” triggers (DARK GREEN, solid)
...
# ±20 are “better/extension” triggers (GREEN, short dash)
...
def fri70ByNoon = isFri and afterNoon and atrUsed[1] < 0.70 and atrUsed >= 0.70;
...
Alert(EnableAlerts and fri70ByNoon,  "Friday >70% ATR after noon — consider closing 0DTE spreads", Alert.BAR, Sound.Bell);
